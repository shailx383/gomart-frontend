<template>
     ...
     <div class="relative flex h-16 items-center justify-between">
       ...
       <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
         <div class="flex flex-shrink-0 items-center">
           <router-link to="/mainman"><img class="img max-h-8" src="../assets/shop.png" alt="" srcset=""></router-link>
            
         </div>
         <div class="sm:ml-6 sm:block">
           <div class="flex space-x-4">
             <h2 class=" font-sansserif text-lg text-white pt-1 ml-3 mr-7"> Edit Product</h2>
   ...
   <div class="pt-6">
       <div class="mx-auto mt-6 max-w-2xl sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-2 lg:gap-x-8 lg:px-8">
                   ...
                   <label for="dropzone-file" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                       <div class="flex flex-col items-center justify-center pt-5 pb-6">
                           ...
                           <p class="mb-2 text-sm text-gray-500 dark:text-gray-400">
                               <span class="font-semibold">Click to upload new image</span> or drag and drop</p>
           ...
           <div class="-space-y-px rounded-md shadow-sm">
             ...
             <div>
               <label for="price" class="block text-sm font-medium text-gray-700 pt-4 pb-3">Price (Old: {{oldProduct.price}})</label>
               <input v-model="newProduct.price" id="price" name="price" type="number" min="0" autocomplete="" required="" class="relative block w-full appearance-none rounded-none rounded-t-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm"/>
             </div>
             <div>
               <label for="q" class="block text-sm font-medium text-gray-700 pt-4 pb-3">Quantity (Old: {{oldProduct.quantity}})</label>
               <input v-model="newProduct.quantity" id="q" name="q" type="number" min="0" autocomplete="" required="" class="relative block w-full appearance-none rounded-none rounded-t-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm" placeholder="" />
             </div>
               <div>
                   <label for="q" class="block text-sm font-medium text-gray-700 pt-4 pb-3">Category (Old: {{oldProduct.category}})</label>
                   <select v-model="newProduct.category" class="form-select appearance-none
                   block
                   w-full
                   px-3
                   py-1.5
             ...
             <div>
               <label for="address" class="block text-sm font-medium text-gray-700 pt-4 pb-3">Delivery time (Old: {{oldProduct.deliveryTime}})</label>
               <input v-model="newProduct.deliveryTime" id="del" min="0" name="del" type="number" autocomplete="" required="" class="relative block w-full appearance-none rounded-none rounded-t-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm" placeholder="" />
             </div>
             <div>
               <label for="offer" class="block text-sm font-medium text-gray-700 pt-4 pb-3">Offer (Old: {{oldProduct.offer}})</label>
               <input v-model="newProduct.offer" id="offer" name="offer" type="number" min="0" max="100" autocomplete="" required="" class="relative block w-full appearance-none rounded-none rounded-t-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm" placeholder="" />
             </div>
           </div>
           <button type="submit" class="group relative flex w-full justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-sm font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
             ...
             <br/>
             <button type="button" @click="deleteProduct()" class="group relative flex w-full justify-center rounded-md border border-transparent bg-red-600 py-2 px-4 text-sm font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">
               Delete Product
             </button>
         </form>
           </div>
   ...
   <div v-if="show_d" class="overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none justify-center items-center flex">
       <div class="relative w-full max-w-md h-full md:h-auto">
           <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
               <button @click = "showDeletion()" type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-toggle="wallet-modal">
                   <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                   <span class="sr-only">Close modal</span>
               </button>
...
<Footer></Footer>
   ...
   export default{
       name: 'EditProdMan',
       components: {AddedProduct, DeletedProduct, Footer, Disclosure, DisclosureButton, DisclosurePanel, Menu, MenuButton, MenuItem, MenuItems, Bars3Icon, BellIcon, XMarkIcon},
       data(){
           return {
               newProduct: {
       ...
       methods: {
           ...
           addProduct(){
                axios.post('https://gomart-production.up.railway.app/admin/updateProduct', {senderId: localStorage.logged, product: this.newProduct})
           ...
           deleteProduct(){
               axios.post('https://gomart-production.up.railway.app/manager/deleteProduct', {senderId: localStorage.logged, product: this.oldProduct})
       ...
       mounted(){
           axios.get('https://gomart-production.up.railway.app/user/products/'+this.$route.params.id)
               .then(resp => {console.log('Old: '+resp.data.name);
