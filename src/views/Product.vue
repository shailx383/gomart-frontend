      ...
      <div class="py-10 lg:col-span-2 lg:col-start-1 lg:border-r lg:border-gray-200 lg:pt-6 lg:pb-16 lg:pr-8">
        <div>
          ...
          <div class="space-y-6">
            ...
            <p class="text-base text-gray-900">Offer: {{ product.offer }}% off</p>
            <p class="text-base text-gray-900">In Stock: {{ product.quantity }}</p>
          </div>
        </div>
      </div>
    </div>
...
<div v-if="addedShown" class="overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none justify-center items-center flex">
              <div class="relative w-full max-w-md h-full md:h-auto">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <button @click = "addedToCart()" type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-toggle="wallet-modal">
      ...
      methods: {
        addToCart(){
          axios.post('https://gomart-production.up.railway.app/user/addToCart', this.purchase)
            .then(response => {console.log(this.purchase);
                                this.addedToCart();
                                })
        },
        addedToCart(){
